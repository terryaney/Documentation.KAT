...
This document describe how we can set up TFS queries, tags, and task states to mimic the KAT TM workflow when creating and working with tasks for projects built on the Evolution framework.  The location for all these queries can be found in the [Conduent TFS Work Area](#https://tfs.acsgs.com/tfs/PDSI/HRS2/KAT/_workitems)

In this document, 'Requestors' are the people who create the tasks, and 'Workers' are the people who implement the tasks.

To enable this, you will create custom queries that filter based on 'KAT tags' describing each client.  `Tags` are used simply due to the fact that we can't create new 'Client Projects' without the help of an admin.  Using a single project (`HRS2/KAT`) with custom `Tags` was the easiest transition.  If you are creating the *first* task for a client in TFS, the pattern of the `Tag` is `KAT {clientName}`.
...
1. If creating a Spec Sheet request task:
	1. Update the Spec Sheet `Requirement` task by uploading the latest Spec Sheet and updating the `Title` to reflect the new version.
	1. Optional; Would be *nice* to copy the ID of the spec task before creating the task so that you are able to create a 'link' of type `Parent` and paste the value of the Spec Sheet task ID.  This makes it easier for Worker to find and download the file.
1. Add correct `Tag` (Remember, `Tag` should be in the format of `KAT {clientName}`).
1. Set `Title` using a prefix of `{clientName} ` (from `Tag` used in previous step).  This aids in the [All Tasks To Do Query](#all-tasks-to-do-query-worker) to easily identify the client associated with the task.
...
This section describes the workflow for implementing and publishing with tasks from the perspective of the Worker.

#### To Start Working on Task

When you start working on a task, simply:
...
If you need additional information from the Requestor who created the task, simply:

1. Set `Assigned To` back to the Requestor (you can find this in the 'History' tab/area of the task if you don't know).  This triggers a notification to the Requestor, but we use `Assigned To` instead of `@id` in `Discussions` to make the [Tasks to Do Query](#tasks-to-do) work.
...
When you have completed a task and are ready to publish and notify the Requestor, simply:

1. Publish to QA
1. Set `Assigned To` back to the Requestor (you can find this in the 'History' tab/area of the task if you don't know).  This triggers a notification to the Requestor, but we use `Assigned To` instead of `@id` in `Discussions` to make their [Tasks to Review Query](#tasks-to-review) work.
...
There maybe be several tasks you plan on finishing before publishing the site and you don't want the Requestor to be notified on each one until you've finished the batch of tasks.  This is the same as the KAT TM's 'Pending Notifications' state.  To accomplish this:
...
When you have completed a batch of tasks and are ready to publish and notify the Requestor (this is the same as clicking 'Send Notification' in KAT TM), simply:
...
1. Go to your [Pending Notifications Query](#pending-notifications-query-worker) and select all tasks you want to notify the Requestor about and:
	1. Set `Assigned To` back to the Requestor (you can find this in the 'History' tab/area of the task if you don't know).  This triggers a notification to the Requestor, but we use `Assigned To` instead of `@id` in `Discussions` to make their [Tasks to Review Query](#to-review-query-requestor) work.
	1. Set `Triage` to Pending (or anything other than Triaged). This will move it out of the 'Pending Notifications Query'.
	1. Set `State` to Resolved.
	1. Save the tasks.

### Publishing to Production

When you have tasks in your [To Publish Query](#tasks-to-publish), simply:
...
This section describes how TFS is used to create 'states' for a task that emulate the KAT TM workflow.  For each client you work on, under the 'My Queries' folder, you will create a folder with the name of the client.  Within that folder, you will create one or more of the queries described below (each being scoped to a `KAT {clientName}` tag).  You will end up with a structure like this:

![My Queries](images/tfs_migration/my-queries.png)
